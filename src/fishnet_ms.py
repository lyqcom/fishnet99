...
PyTorch model from https://github.com/zsef123/Fishnet-PyTorch.
"""
...
def _bn_relu_conv(in_c, out_c, **conv_kwargs):
    return nn.SequentialCell([nn.BatchNorm2d(num_features=in_c, momentum=0.9),  # eps=0.001
                              nn.ReLU(),
...
class ResBlock_with_shortcut(nn.Cell):  # 6_1
    """
    ...
    """
    def __init__(self, in_c, out_c, shortcut, stride=1, dilation=1):  # shortcut
...
class ResBlock_without_shortcut(nn.Cell):  # 6_2
    """
    Construct Basic Bottle-necked Residual Block module.
    Args:
        in_c : Number of channels in the input image
    ...
    Forwarding Path:
        ...
        input image - (BN-ReLU-Conv) * 3 - (add) -output
    """
    def __init__(self, in_c, out_c, stride=1, dilation=1):  # no shortcut
...
class TransferBlock(nn.Cell):  # 10
    """
    ...
    def __init__(self, ch, num_blk):
        ...
        self.layers_TransferBlock = nn.SequentialCell([*[ResBlock_without_shortcut(ch, ch)
                                                         for _ in range(0, num_blk)]])  # 10 ==> 6_2
...
class DownStage(nn.Cell):  # 5
    ...
    def __init__(self, in_c, out_c, num_blk, stride=1):
        ...
        self.layer_DownStage = nn.SequentialCell([
            ResBlock_with_shortcut(in_c, out_c, shortcut),  # 5 ==> 6_1
...
class UpStage(nn.Cell):  # 9
    ...
    def __init__(self, in_c, out_c, num_blk, dilation=1):
        ...
        self.layer_UpStage = nn.SequentialCell([
            ResBlock_with_shortcut(in_c, out_c, channel_reduction_ms(in_c // out_c), dilation=dilation),  # //// 9 ==> 6_1
            *[ResBlock_without_shortcut(out_c, out_c, dilation=dilation) for _ in range(1, num_blk)],  # 9 ==> 6_2
        ])
...
class FishTail(nn.Cell):  # 4
    ...
    def __init__(self, in_c, out_c, num_blk):
        ...
        self.layer_FishTail = DownStage(in_c, out_c, num_blk)  # 4 ==> 5
...
class Bridge(nn.Cell):  # 7
    ...
    def __init__(self, ch, num_blk):
        ...
        self.layers_Bridge = nn.SequentialCell([
            ResBlock_with_shortcut(ch * 2, ch, shortcut),  # 7 ==> 6_1
            *[ResBlock_without_shortcut(ch, ch) for _ in range(1, num_blk)],  # 7 ==> 6_2
        ])
...
class FishBody_0(nn.Cell):  # 8
    ...
    def __init__(self, in_c, out_c, num_blk, trans_in_c, num_trans, dilation=1):
        ...
        self.layer_FishBody = UpStage(in_c, out_c, num_blk, dilation=dilation)  # 8 ==> 9
        self.add_up = par_ms_0()  # ///
        self.transfer = TransferBlock(trans_in_c, num_trans)  # 8 ==> 10
...
class par_ms_0(nn.Cell):  # ///
    ...
    def __init__(self):
        ...
        self.P_up = P.ResizeNearestNeighbor((14, 14))  # only Ascend has P.ResizeBilinear()
...
class FishHead(nn.Cell):  # 11
    ...
    def __init__(self, in_c, out_c, num_blk, trans_in_c, num_trans):
        ...
        self.layer_FishHead = nn.SequentialCell([
            ResBlock_without_shortcut(in_c, out_c),  # 11 ==> 6_2
            *[ResBlock_without_shortcut(out_c, out_c) for _ in range(1, num_blk)],  # 11 ==> 6_2
            nn.MaxPool2d(kernel_size=2, stride=2, pad_mode='valid')
        ])
        self.transfer = TransferBlock(trans_in_c, num_trans)  # 11 ==> 10
...
def _conv_bn_relu(in_ch, out_ch, stride=1):
    return nn.SequentialCell([nn.Conv2d(in_channels=in_ch, out_channels=out_ch, kernel_size=3, stride=stride,
                              ...
                              nn.BatchNorm2d(num_features=out_ch, momentum=0.9),  # eps=0.001
                              nn.ReLU()])


class Fishnet(nn.Cell):  # 3
    """
    ...
    def __init__(self, start_c=64, num_cls=1000,
                 tail_num_blk=1, bridge_num_blk=2,  # [2, 2, 6], 2
                 body_num_blk=1, body_num_trans=1,  # [1, 1, 1], [1, 1, 1]
                 head_num_blk=1, head_num_trans=1,  # [1, 2, 2], [1, 1, 4]
                 tail_channels=1, body_channels=1, head_channels=1):  # tail_channels=[64, 128, 256, 512],
        # body_channels=[(512, 512), (768, 384), (512, 256)],
        # head_channels=[(320, 320), (832, 832), (1600, 1600), (2112, 2112)]
        super(Fishnet, self).__init__()  # ////

        self.stem = nn.SequentialCell([
            _conv_bn_relu(3, start_c // 2, stride=2),  # in_ch:3, out_ch:32
            _conv_bn_relu(start_c // 2, start_c // 2),  # in_ch:32, out_ch:32
            _conv_bn_relu(start_c // 2, start_c),  # in_ch:32, out_ch:64
            nn.MaxPool2d(kernel_size=3, stride=2, pad_mode='same')  # out_ch:64
        ])
        print("FishNet Initialization Start")

        self.tail_layer = []  # ////
        for i, num_blk in enumerate(tail_num_blk):  # [2, 2, 6]
            layer = FishTail(tail_channels[i], tail_channels[i + 1], num_blk)  # 3 ==> 4
            self.tail_layer.append(layer)
        self.tail_layer = nn.CellList(self.tail_layer)  # ///
        self.bridge = Bridge(tail_channels[-1], bridge_num_blk)  # 3 ==> 7

        self.body_layer = []  # ////
        # zip([1,2],[3,4]) = [(1,3),(2,4)]
        for i, (num_blk, num_trans) in enumerate(zip(body_num_blk, body_num_trans)):
            if i == 0:
                layer = FishBody_0(body_channels[i][0], body_channels[i][1], num_blk,  # 3 ==> 8
                                   tail_channels[-i - 2], num_trans, dilation=2 ** i)
            elif i == 1:
                layer = FishBody_1(body_channels[i][0], body_channels[i][1], num_blk,  # 3 ==> 8
                                   tail_channels[-i - 2], num_trans, dilation=2 ** i)
            else:
                layer = FishBody_2(body_channels[i][0], body_channels[i][1], num_blk,  # 3 ==> 8
                                   tail_channels[-i - 2], num_trans, dilation=2 ** i)
        ...
        for i, (num_blk, num_trans) in enumerate(zip(head_num_blk, head_num_trans)):
            layer = FishHead(head_channels[i][0], head_channels[i][1], num_blk,  # 3 ==> 11
        ...
        self.classifier = nn.SequentialCell([
            nn.BatchNorm2d(num_features=last_c, momentum=0.9),  # eps=0.001
            nn.ReLU(),
            nn.Conv2d(in_channels=last_c, out_channels=last_c // 2, kernel_size=1, pad_mode='pad', has_bias=False
                      , weight_init=conv_weight_init),
            nn.BatchNorm2d(num_features=last_c // 2, momentum=0.9),  # eps=0.001
            nn.ReLU(),
            adaptiveavgpool2d_ms(),  # ////
...
# net_factory.py//////////////////////////////////////////////////////////////////
def _calc_channel(start_c, num_blk):  # 2
    """
    ...
    print("Tail Channels : ", tail_channels)  # [64, 128, 256, 512]
    ...
    print("Body Channels : ", body_channels)  # [(512, 512), (768, 384), (512, 256)]
    ...
    print("Head Channels : ", head_channels)  # [(320, 320), (832, 832), (1600, 1600), (2112, 2112)]
    return {"tail_channels": tail_channels, "body_channels": body_channels, "head_channels": head_channels}


def fish99(num_cls=1000):  # 1
    """fish99"""
    start_c = 64
    # tail
    tail_num_blk = [2, 2, 6]
    ...
    net_channel = _calc_channel(start_c, len(tail_num_blk))  # 1 ==> 2

    # 1 ==> 3
    return Fishnet(start_c, num_cls,  # 64, 1000
                   tail_num_blk, bridge_num_blk,  # [2, 2, 6], 2
                   body_num_blk, body_num_trans,  # [1, 1, 1], [1, 1, 1]
                   head_num_blk, head_num_trans,  # [1, 2, 2], [1, 1, 4]
                   **net_channel)  # {"tail_channels": [64, 128, 256, 512],
    # "body_channels": [(512, 512), (768, 384), (512, 256)],
    # "head_channels": [(320, 320), (832, 832), (1600, 1600), (2112, 2112)]}


# def fish150(num_cls=1000):  # 1
#     start_c = 64
#
#     tail_num_blk = [2, 4, 8]
#     bridge_num_blk = 4
